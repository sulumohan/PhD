...
def example_traces(ax):
    ...
    # Plot Ch0 data
    df_ch0 = df_filtered.loc[:, 'Ch0_4000':'Ch0_5500']
    ...
    # Plot Ch3 data
    df_ch3 = df_filtered.loc[:, 'Ch3_4000':'Ch3_4800']
...
def plot_peakP1(ax):
    ...
    for idx, numSQ in enumerate([1, 5, 15]):
        ...
        for i, df in enumerate(dfs_allcells):
            ...
            grouped_data = filtered_df.groupby('numPattern')['peakP1'].apply(list).reset_index()
    
            x = grouped_data['numPattern']
            y = grouped_data['peakP1']
            color_for_df = plt.cm.tab10(i / len(dfs_allcells))
    
            # Plot each peakP1 value
            for pattern_id, peakP1_values in zip(x, y):
                ax[idx].scatter([pattern_id] * len(peakP1_values), peakP1_values, color=color_for_df, label=f'Cell {i+1}' if idx == 0 else "", alpha=0.7, s=150)
...
def field_vs_P1(ax):
    ...
    for i, (clamp_potential, title) in enumerate([(-70, 'Excitation'), (0, 'Inhibition')]):
        ...
        ax_.set_xlabel('Field P1', fontsize=16)
        ax_.set_ylabel('Cell P1', fontsize=16)
...
def cell_physio_vs_P1(ax):
    ...
    for i, clampPotential in enumerate(conditions['clampPotential']):
        ...
        ax[2*i].set_ylabel('PeakP1', fontsize=14)
        ...
        for k, df in enumerate(dfs_allcells):
            ...
            if not df_filtered.empty:
                ...
                ax[2*i].scatter(df_filtered['holdI'], df_filtered['peakP1'], color=color, alpha=0.7)
                
                # Linear regression
                slope, intercept, r_value, p_value, std_err = linregress(df_filtered['holdI'], df_filtered['peakP1'])
        ...
        ax[2*i+1].set_ylabel('PeakP1', fontsize=14)
        ...
        for k, df in enumerate(dfs_allcells):
            ...
            if not df_filtered.empty:
                ...
                # Scatter plot with points
                ax[2*i+1].scatter(df_filtered['IR'], df_filtered['peakP1'], color=color, alpha=0.7)
                
                # Linear regression
                slope, intercept, r_value, p_value, std_err = linregress(df_filtered['IR'], df_filtered['peakP1'])
...
def glm_cell_physio(ax):
    ...
    model = sm.GLM.from_formula(
        'peakP1 ~ C(numSQ) + holdI + IR + desensitization + bln_std_cell + snr_cell',
